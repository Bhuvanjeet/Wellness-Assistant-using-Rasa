...
class HealthForm(FormAction):
    ...
    def required_slots(tracker):
        ...
        if tracker.get_slot('confirm_exercise') == True: #to check whether exercise slot has already been filled: if so, ask the user about it
            return ["confirm_exercise", "exercise", "sleep","diet", "stress", "goal"]
        else:                           #request all the slots except for the exercise slot
    ...
    def submit(                 #gives opportunity to execute whatever we want once all the slots required by the form are filled
        self,
        dispatcher: CollectingDispatcher,  #used to send back msgs to the user
